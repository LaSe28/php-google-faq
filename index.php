...
<div class="container">
  <?php 
    $faq = [
      ...
      [
        'quest' => 'Perché il mio account è associato a un paese?',
        'listAnsw1' => [
          'answ1' => 'Il tuo account è associato a un paese (o territorio) nei Termini di servizio per poter stabilire due cose:',
          'item1' => 'La società consociata Google che offre i servizi, tratta le tue informazioni ed è responsabile del rispetto delle leggi sulla privacy vigenti. Generalmente Google offre i propri servizi per i consumatori tramite una delle due società seguenti:',
          'listAnsw2' => [
          ...
          'item2' => 'La versione dei termini che regola il nostro rapporto, che può variare in base alle leggi locali.',
          'answ2' => 'Tieni presente che i servizi Google sono fondamentalmente gli stessi a prescindere dalla società consociata che li offre o dal paese a cui è associato il tuo account.',
          ],
        'subquest' => 'Stabilire il paese associato al tuo account',
        'subansw' => [
      ...
      [
        ...
        'answ' => 'I risultati di ricerca di Google rispecchiano i contenuti pubblicamente disponibili sul Web. I motori di ricerca non possono rimuovere i contenuti direttamente dai siti web, quindi rimuovere risultati di ricerca da Google non consente di rimuovere i contenuti dal Web. Se desideri rimuovere qualcosa dal Web, devi contattare il <a href="#!">webmaster</a> del sito su cui sono pubblicati i contenuti e chiedergli di apportare una modifica. Inoltre, se, ai sensi delle leggi europee per la protezione dei dati, desideri richiedere la rimozione di determinate informazioni su di te visualizzate nei risultati di ricerca di Google, fai <a href="#!">clic qui</a>. Una volta che i contenuti saranno stati rimossi e che Google avrà rilevato l\'aggiornamento, le informazioni non verranno più visualizzate nei risultati di ricerca di Google. In caso di una richiesta di rimozione urgente, è inoltre <a href="#!">possibile visitare la nostra pagina di assistenza per avere ulteriori informazioni.</a>',
      ],
      [
    ...
    foreach ($faq as $item) {
      echo "<h2> {$item['quest']}</h2>";
      if(in_array('answ', array_keys($item))){
        if (is_array($item['answ'])){
          foreach($item['answ'] as $parag) {
            echo "<p> {$parag} </p>";
          }
        } else {
          echo "<p> {$item['answ']}";
      ...
      if(in_array('listAnsw1', array_keys($item))){
  ?>
      ...
      <p><?=$item['listAnsw1']['answ2']?></p>
  <?php
      }
      if(in_array('subquest', array_keys($item))){
        echo '<h3>' . $item['subquest'] . '</h3>';
      }
      if(in_array('subansw', array_keys($item))){
        foreach($item['subansw'] as $parag) {
          echo "<p> {$parag} </p>";
        }
      }
    }
